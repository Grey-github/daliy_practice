...
Game::Game()
{
    ...
    start_color(); // Use color for our terminal, make sure your teminal has color support
    curs_set(8); // Make cusor invisible
    // Create a ncurese window
    ...
    nodelay(m_window, true); // This causes like getch() or wgetch() to be a non_blocking call
    ...
    m_spaceship = Entity(PLAYGROUND_WIDTH / 2 + PLAYGROUND_POS_X, // Centered vertical
                                                   ...
                                                   3, 1, 'H', 10); // The spaceship is a 3*1 size 10 life entity with the body represented by 'H'
...
void Game::Input()
{
    // Get a user input character, wgetch() will not block the program, cause we are in the delay mode
    ...
    case KEY_RIGHT:
    case 'd':
        if (m_spaceship.GetX() < PLAYGROUND_POS_X - 2)
            m_spaceship.SetX(m_spaceship.GetX() + 1);
        break;
...
void Game::Render()
{
    werase(m_window); // Clear screen immdiately
    /* Render rockd */
    // Render tiny rocks
    ...
    wattroff(m_window, COLOR_PAIR(2)); // Blue 

    /* Render UI */
    wattron(m_window, A_BOLD);
    box(m_window, 0, 0); // Draw a box around the edge of the window
    wattron(m_window, COLOR_PAIR(2)); // Blue
    mvwaddstr(m_window, BORDER_POS_Y, 20, "[Space Game]"); // Draw title
    ...
    wrefresh(m_window); // Get actual output to the teminal
}

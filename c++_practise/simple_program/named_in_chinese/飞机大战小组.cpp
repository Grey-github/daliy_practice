...
void Drawenemy(COORD* position)
{
	for (int i = 0; i < 5; i++)  //随机生成五架敌机
	{
		...
		if (position[i].Y >= 24)
		{
			...
			{
				flag = 0;
				position[i].Y = rand() % 40 + 3;
				position[i].X = rand() % 40 + 4 + i;//随机取五架敌机生成的位置
...
int main()
{
	...
	system("color 8E");
	cout<<endl;
	cout<<"   ----------w s a d 控制战机移动 k发射子弹-------------"<<endl;
	cout<<'\t'<<'\t'<<"*****************************"<<endl;
	cout<<'\t'<<'\t'<<"|      您是否要开始游戏     |"<<endl;
	cout<<'\t'<<'\t'<<"|  选项： 1 开始游戏        |"<<endl;
	cout<<'\t'<<'\t'<<"|         2 退出            |"<<endl;
	...
	int counter = 5;//用于保证自身飞机速度不受影响的同时延缓敌机速度
	HideCursor();
	//Drawwall();
	COORD bullet[10];
	...
	while (GameOver(position, enemyposition))//将飞机的移动和敌机的移动放进同一循环，以达到同时移动的目的
	{
		...
		if (counter % 4 == 0)//这里通过每循环6次敌机才移动一次，保证在自己飞机移动速度不受影响、不卡顿的同时使敌机速度减慢
